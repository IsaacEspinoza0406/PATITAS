...
//    }
    ...
    suspend fun getById(request: DogsRequest){
        val result = dbQuery {
            val selectStatement = DogsTable.select { DogsTable.id eq request.id }
        }
        return result ?: throw IllegalStateException("Error al obtener el ID del perro de la base de datos")
    }
